
public class LocalizacaoHandler extends AbstractHandler {

    @Override
    public void handleRequest(double valor, String localizacao, String historico) {
        if (canHandle(valor, localizacao, historico)) {
            System.out.println("LocalizacaoHandler processou a requisição: ");
            nextHandler.handleRequest(valor, localizacao, historico);
        } else {
            System.out.println("LocalizacaoHandler: Transação suspeita devido à localização: " + localizacao);
            System.out.println("Valor: " + valor);
            System.out.println("Localizacao: " + localizacao);
            System.out.println("Historico: " + historico);
        }
    }

    private boolean canHandle(double valor, String localizacao, String historico) {
        return localizacao.equals("Brasil");
    }

}
